...
if page=="简介页面":
    ...
    with col_form:
        ...
        with st.form('user_inputs'):
            island=st.selectbox('企鹅栖息岛屿',options=['托尔槮岛','比斯科群岛','德里姆岛'])
            sex=st.selectbox('性别',options=['雄性','雌性'])
            
            bill_length=st.number_input('缘的长度(毫米)',min_value=0.0)
            bill_depth=st.number_input('缘的长度（毫米）',min_value=0.0)
    ...
    #根据用户输入的性别数据更改对应的值
        ...
        #使用pickle的load方法从磁盘文件反序列化加载一个之前保存的映射现象
